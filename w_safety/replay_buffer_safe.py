...
class ReplayBuffer:
    def __init__(self, max_size, input_shape): #, n_actions
        ...
        self.safety_mem = np.zeros(self.mem_size)
    ...
    def store_safety(self, safety):
        index = (self.mem_cntr-1) % self.mem_size
        self.safety_mem[index] = safety

    def sample_safety(self, batch_size):
        max_mem = min(self.mem_cntr, self.mem_size)
        batch = np.random.choice(max_mem, batch_size)
        safety = self.safety_mem[batch]
        return safety