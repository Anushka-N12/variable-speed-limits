...
class MetaNetEnv:
    ...
    def step(self, action):
        ...
        # self.prev_speed = getattr(self, 'prev_speed', float(self.v[0]))
        decel = max(0, prev_speeds - self.v)
        safety_reward = -decel / 10  # Scale appropriately
        # self.prev_speed = float(self.v[0])

        self.time += 1
        done = self.time >= self.timesteps

        return next_state, reward, done, {}, safety_reward
